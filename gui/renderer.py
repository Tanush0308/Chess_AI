...
def draw_captured_pieces(screen, gs):
    ...
    for move in gs.move_log:
        if move.piece_captured != "--":
            if move.piece_captured[0] == "w":
                white_captured.append(move.piece_captured)
            else:
                black_captured.append(move.piece_captured)

    piece_size = 28
    x_start = 650
    max_width = 780   # right boundary
    ...
    screen.blit(font.render("Black lost:", True, (0,0,0)), (650, 100))
    screen.blit(font.render("White lost:", True, (0,0,0)), (650, 180))
    ...
    for piece in black_captured:
        ...
        if x + piece_size > max_width:
            x = x_start
            y += piece_size + 5

        img = pygame.transform.scale(IMAGES[piece], (piece_size, piece_size))
        screen.blit(img, (x, y))
        x += piece_size + 5
